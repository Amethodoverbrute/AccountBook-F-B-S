...
| **ORM 框架** | Mongoose  | 9.x  | MongoDB 对象模型/关系映射 |
| **认证机制** | JWT       | 9.x  | 用户身份认证              |
...
1. **数据模型定义**  
   Mongoose 允许你通过 **Schema**（模式）来定义 数据结构，**类似于**关系型数据库的 表结构。例如：
...
JWT（JSON Web Token）是一种用于在网络应用间 传递声明 的基于 JSON 的开放标准。
...
│   └── error.log     # 错误日志
├── middlewares/      # 中间件
│   ├── checkLoginMiddleware.js       # 登录检查中间件（适用于Web路由，当前未使用）
│   ├── checkTokenMiddleware.js       # Token验证中间件（适用于API路由）
│   ├── loggerMiddleware.js           # 日志中间件
│   ├── checkAdminMiddleware.js       # 管理员权限验证中间件
│   └── checkSuperAdminMiddleware.js  # 超级管理员权限验证中间件
├── models/           # 数据模型
│   ├── accountModel.js   # 账单模型
│   ├── categoryModel.js  # 分类模型
│   └── userModel.js      # 用户模型（添加了role字段）
├── routes/           # 路由
│   ├── api/          # API路由（当前项目使用的路由）
│   │   ├── account.js     # 账单API
│   │   ├── auth.js        # 认证API
│   │   ├── category.js    # 分类API
│   │   ├── statistics.js  # 统计API
│   │   └── admin.js       # 管理员API（用户管理、系统统计）
│   └── web/          # Web路由（传统SSR路由，当前未使用）
│       ├── auth.js        # Web认证路由
│       └── index.js       # Web首页路由
...
| 对比项       | API 路由 (routes/api/)           | Web 路由 (routes/web/)     |
| ------------ | -------------------------------- | -------------------------- |
| **架构设计** | 前后端分离架构                   | 传统服务器端渲染(SSR)架构  |
| **响应格式** | 返回 JSON 格式数据               | 渲染 HTML 页面或返回文本   |
| **认证方式** | 使用 JWT 令牌认证                | 使用 Session 会话认证      |
| **文档支持** | 包含 Swagger 文档注释(@openapi)  | 无 Swagger 注释            |
| **响应方式** | 使用`res.json()`返回数据         | 使用`res.render()`渲染页面 |
| **路由前缀** | 挂载在`/api`路径下               | 直接挂载在根路径`/`下      |
| **前端交互** | 由前端 JavaScript 发起 AJAX 请求 | 由浏览器直接请求 URL       |

**当前项目使用情况**：

- 仅使用**API 路由**，通过`app.js`中的`app.use('/api', router)`注册
...
- `src/router/index.js`：定义前端路由，包括管理员路由
...
- `src/components/pages/AdminLogin.vue`：管理员登录组件
- `src/components/pages/AdminDashboard.vue`：后台管理仪表盘
- `src/components/pages/UserManagement.vue`：用户管理页面
- `src/components/pages/SystemStatistics.vue`：系统统计页面
...
- `src/services/auth.js`：封装认证相关 API 请求
- `src/services/admin.js`：封装管理员相关 API 请求
...
- `app.js`：后端应用入口，配置中间件和路由，包括管理员路由
- `config.js`：根目录配置文件
- `models/accountModel.js`：账单数据模型定义
- `models/userModel.js`：用户数据模型，添加了 role 字段
- `routes/api/auth.js`：用户认证相关 API（当前使用）
- `routes/api/admin.js`：管理员相关 API（用户管理、系统统计）
- `middlewares/checkTokenMiddleware.js`：JWT 令牌验证（当前使用）
- `middlewares/checkLoginMiddleware.js`：登录检查中间件（<font color='gree'>适用于 Web 路由，当前未使用</font>）
- `middlewares/loggerMiddleware.js`：日志中间件（当前使用）
- `middlewares/checkAdminMiddleware.js`：管理员权限验证中间件
- `middlewares/checkSuperAdminMiddleware.js`：超级管理员权限验证中间件
- `config/swagger.js`：Swagger 文档配置（当前使用）
- `config/logger.js`：日志配置（当前使用）
- `routes/api/statistics.js`：统计数据 API（当前使用）
- `routes/web/auth.js`：Web 认证路由（<font color='gree'>传统 SSR 路由，当前未使用</font>）
- `routes/web/index.js`：Web 首页路由（<font color='gree'>传统 SSR 路由，当前未使用</font>）
...
- **核心流程**：
  ...
  6. 路由守卫检查用户角色权限
- **核心文件**：
  - 前端：`src/components/pages/AdminLogin.vue`
  - 后端：`routes/api/auth.js`、`middlewares/checkTokenMiddleware.js`
...
- **功能**：处理系统用户的管理，包括查看、修改角色和删除
- **关键技术**：角色优先级排序、数据表格、权限控制
- **核心流程**：
  1. 管理员登录后台
  2. 查看用户列表（按角色优先级排序：superAdmin > admin > user）
  3. 超级管理员可以修改用户角色或删除用户
  4. 管理员只能查看用户列表
- **核心文件**：
  - 前端：`src/components/pages/UserManagement.vue`
  - 后端：`routes/api/admin.js`、`middlewares/checkAdminMiddleware.js`、`middlewares/checkSuperAdminMiddleware.js`
...
- **核心流程**：
  1. 管理员登录后台
  2. 查看系统统计数据
  3. 包括总用户数、总账单数、日活跃用户数
- **核心文件**：
  - 前端：`src/components/pages/SystemStatistics.vue`
  - 后端：`routes/api/admin.js`
...
| 管理员获取用户列表 | GET       | /api/admin/users          |
| 管理员修改用户角色 | PUT       | /api/admin/users/:id/role |
| 管理员删除用户     | DELETE    | /api/admin/users/:id      |
| 管理员获取系统统计 | GET       | /api/admin/statistics     |
...
2. ==查看`fronted/src/services/auth.js`，了解 API 请求封装==
...
1. 查看`src/router/index.js`，了解管理员路由配置
2. 学习`src/components/pages/AdminLogin.vue`，理解管理员登录流程
3. 查看`src/components/pages/AdminDashboard.vue`，了解后台管理布局
4. 学习`src/components/pages/UserManagement.vue`，理解用户管理功能
5. 查看`src/components/pages/SystemStatistics.vue`，了解系统统计功能
6. 学习`src/services/admin.js`，了解管理员 API 封装
7. 查看`middlewares/checkAdminMiddleware.js`和`middlewares/checkSuperAdminMiddleware.js`，理解权限控制
8. 学习`routes/api/admin.js`，理解后端管理员 API 实现
...
**重点**：<font color='red'>理解如何将复杂组件**拆分**为多个独立子组件</font>
...
1. 查看`middlewares/checkAdminMiddleware.js`和`middlewares/checkSuperAdminMiddleware.js`
2. 学习`src/router/index.js`中的路由守卫实现
3. 理解`src/components/pages/UserManagement.vue`中的权限控制
4. 学习如何设计和实现角色权限系统
...
module.exports = async (req, res, next) => {
  try {
    ...
    if (!user || (user.role !== "admin" && user.role !== "superAdmin")) {
      return res.status(403).json({ code: "403", msg: "没有权限访问此资源" });
    }
    next();
  } catch (error) {
...
1. 查看`routes/api/admin.js`中的用户列表查询实现
2. 学习 MongoDB 聚合管道的使用
3. 理解 $addFields 和 $sort 操作符的使用
4. 学习如何设计和实现复杂的排序逻辑
...
1. 查看`src/components/pages/AdminDashboard.vue`的布局实现
2. 学习 CSS Flexbox 或 Grid 布局
3. 理解响应式设计原则
4. 学习如何设计用户友好的后台管理界面
...
2. `src/router/index.js` - 了解路由配置，包括管理员路由
...
9. `src/components/pages/AdminLogin.vue` - 了解管理员登录流程
10. `src/components/pages/AdminDashboard.vue` - 了解后台管理布局
11. `src/components/pages/UserManagement.vue` - 了解用户管理功能
12. `src/services/admin.js` - 了解管理员 API 封装
...
6. `routes/api/admin.js` - 了解管理员 API
7. `middlewares/checkAdminMiddleware.js` - 了解管理员权限验证
8. `middlewares/checkSuperAdminMiddleware.js` - 了解超级管理员权限验证
9. `models/userModel.js` - 了解用户模型（添加了 role 字段）
...
6. **后台管理系统实现**：添加了完整的后台管理功能，包括管理员登录、用户管理和系统统计
7. **角色权限管理**：实现了基于角色的权限控制，区分 superAdmin、admin 和 user 角色
8. **用户排序优化**：实现了按角色优先级排序的用户列表
9. **统一的危险按钮样式**：所有危险按钮（删除按钮）统一使用红色样式，提高用户操作辨识度
10. **响应式布局设计**：优化了页面布局，确保在不同设备上都有良好的显示效果
11. **页面标题和搜索框居中**：优化了页面布局，使标题和搜索框居中显示，提升视觉体验
...
6. **权限控制**：实现了基于角色的细粒度权限控制，区分 superAdmin、admin 和 user 角色
7. **CORS 安全配置**：在生产环境中限制允许的来源，只允许特定域名访问
8. **API 限流**：实现 API 限流，防止恶意请求
9. **敏感数据保护**：在返回用户数据时排除密码等敏感字段
